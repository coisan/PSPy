import time
from holdem_master import holdem_functions, holdem_argparser
import numpy as np
...
def run_simulation(hole_cards, num, exact, given_board, deck, verbose):
    ...
    if (None, None) in hole_cards:
        ...
        unknown_indexes = indices(hole_cards, (None, None))
        for filler_hole_cards in holdem_functions.generate_hole_cards_random(deck, len(unknown_indexes), num):
            idx = 0
            for card_index in unknown_indexes:
                hole_cards_list[card_index] = (filler_hole_cards[idx], filler_hole_cards[idx+1])
                deck_list = list(deck)
                deck_list.remove(filler_hole_cards[idx])
                deck_list.remove(filler_hole_cards[idx+1])
                idx = idx + 2
